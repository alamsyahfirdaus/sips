   	...
   	<div class="box">
   		<div class="box-header with-border">
   			...
   			<div class="box-tools pull-right">
   				<?php if (!@$row->user_id): ?>
   				<a href="javascript:void(0)" class="btn btn-box-tool" onclick="reset_validator();"><i class="fa fa-refresh"></i></a>
   				<?php endif ?>
   			</div>
   		</div>
	   		...
	   		<div class="box-body">
		   		...
		   		<div class="row">
		   			...
		   			<div class="col-md-6 col-xs-12">
		   				...
		   				<div class="form-group">
		   				  <label for="status_guru">Status Kepegawaian</label>
		   				  <select name="status_guru" id="status_guru" class="form-control select2">
		   				  	<option value="">Status Kepegawaian</option>
		   				  	<?php foreach ($this->include->status_guru() as $key): ?>
		   				  		<option value="<?= $key ?>" <?php if($key == @$row->status_guru) echo 'selected'; ?>><?= $key ?></option>
		   				  	<?php endforeach ?>
		   				  </select>
		   				  <small class="help-block"></small>
		   				</div>
...
<script type="text/javascript">
		...
		.bootstrapValidator({
		  ...
		  fields: {
		    ...
		    status_guru: {
		      validators: {
		          notEmpty: {
		              message: 'Status Kepegawian harus diisi'
		          },
		      }
		    },
		    email: {
	...
	function btn_back() {
		window.location.href = index + 'user/teacher';
	}
